...
def show_appreciation_forecast(df):
    ...
    for scenario in scenarios:
        ...
        total_appreciation = (final_price - last_price) / last_price * 100
    ...
    if 'city' in df.columns and 'last_sold_date' in df.columns and 'price' in df.columns:
        ...
        if len(recent_years) >= 2:
            ...
            # Calculate average prices by city for each year
            previous_prices = df_with_year[df_with_year['year'] == previous_year].groupby('city')['price'].mean()
            current_prices = df_with_year[df_with_year['year'] == current_year].groupby('city')['price'].mean()
            
            # Calculate appreciation rates
            appreciation_rates = pd.DataFrame({
                'previous_price': previous_prices,
                'current_price': current_prices
            }).dropna()
            
            appreciation_rates['appreciation'] = (appreciation_rates['current_price'] - appreciation_rates['previous_price']) / appreciation_rates['previous_price'] * 100
            appreciation_rates = appreciation_rates.sort_values('appreciation', ascending=False).reset_index()
...
def show_market_indicators(df):
    ...
    if 'city' in df.columns and 'price' in df.columns:
        ...
        # Rename columns
        city_metrics.columns = ['City', 'Median Price', 'Avg. Days on Market' if 'days_on_market' in df.columns else 'Count', 
                              'Median Sqft' if 'sqft' in df.columns else 'Count']
